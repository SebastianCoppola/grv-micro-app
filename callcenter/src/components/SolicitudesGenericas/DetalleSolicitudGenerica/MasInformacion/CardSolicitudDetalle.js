...
const CardSolicitud = (props) => {
    const { solicitud, aResponder, aAsignar, expandir, handleAsignarGestor, handleResponderMasInfo } = props
    const classes = useStyles()
    const [openCard, setOpenCard] = useState(expandir)
                ...
                <Grid container item justify="space-between" alignItems="center" spacing={1}>
                    <Grid item xs>
                        <Typography style={{ fontSize: 17 }}>
                            {`Solicitud #${solicitud && solicitud.idSolicitudMasInfo}`}
                        </Typography>
                    </Grid>
                    <Grid item>
                    ...
                    <Grid item>
                        <Box display="flex" alignItems="center" ml={1}>
                            ...
                            <CustomButton
                                height={30}
                                label={solicitud && solicitud.descripcionEstado
                                ...
                                <Grid item>
                                    <Box display="flex" flexWrap="wrap">
                                        ...
                                        {solicitud && solicitud.adjuntoSolicitud
                                            ? <Box flexGrow={1}><ArchivoAdjunto nombre={solicitud && solicitud.adjuntoSolicitud ? solicitud.adjuntoSolicitud : "Nombre del archivo.pdf"} peso="-" masInfo={true} /></Box>
                        ...
                        <Grid item>
                            <Box display="flex" alignItems="center">
                                ...
                                {solicitud && (solicitud.gestorSolicitado || solicitud.areaGestion) ?
                                    <CustomButton
                                        height={30}
                                        label={solicitud.gestorSolicitado ? solicitud.gestorSolicitado : solicitud.areaGestion}
                                        startIcon={<PersonIcon />}
                                        styleButton={{ boxSizing: 'border-box', border: '1px solid #cacaca', backgroundColor: '#f5f5f5' }}
                                        styleLabel={{ textTransformation: 'none', color: '#6e6e6e' }}
                                        disabled={!aAsignar}
                                        onClik={() => handleAsignarGestor(solicitud)}
                                    />
                                    : '-'}
                            </Box>
                        </Grid>
                            ...
                            <>
                                        ...
                                        <Box style={{ border: 'solid 1px #f2f2f2' }}>
                                            <Box minHeight={100} display="flex" justifyContent="center" alignItems="center">
                                                <CustomButton
                                                    ...
                                                    onClik={() => handleResponderMasInfo(solicitud)}
                                                />
                                            </Box>
                                        </Box>
                                        :
                                ...
                                <Grid item >
                                    <Box display="flex" flexWrap="wrap">
                                        ...
                                        {solicitud && solicitud.adjuntoRespuesta
                                            ? <Box flexGrow={1}><ArchivoAdjunto nombre={solicitud && solicitud.adjuntoRespuesta ? solicitud.adjuntoRespuesta : "Nombre del archivo.pdf"} peso="15 Kb" masInfo={true} /></Box>
                                            : <Typography style={{ fontSize: 14 }}>Sin adjunto</Typography>
                                        }
                                    </Box>
                                </Grid>
