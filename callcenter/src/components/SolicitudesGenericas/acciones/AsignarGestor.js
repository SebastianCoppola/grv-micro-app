...
const AsignarGestor = (props) => {
    const { gestor, setGestor, buttonLabel, titulo, confirmarHandler = () => { }, actualizarData } = props
    ...
    const botonConfirmacionConfig = botonConfirmacionConfigFn(
        ...
        () => {
            ...
            let cb = (snack, bool) => {
                let message = bool 
                    ? `Solicitud Genérica ASIGNADA a ${gestorDescripcion && gestorDescripcion.length > 0 && gestorDescripcion[0] && gestorDescripcion[0].nombreApellido}.`
                    : 'Hubo un error al asignar la solicitud generica a un gestor.'
                setGestor(null)
                dispatch(actions.actualizarDataSG(!actualizarData))
                dispatch(actions.setSnackBar({open:true, message:message, severity:snack}))
            }
            let req = {
                "idOperador": gestor,
