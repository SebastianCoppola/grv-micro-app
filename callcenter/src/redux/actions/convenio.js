import {
    ...
    SET_CONVENIO_FUTURO, SET_LOADING_CONVENIO_FUTURO, SET_ERROR_CONVENIO_FUTURO,SET_PRESTACIONES_PREQUIRUGICAS,
    SET_LOADING_PRESTACIONES_PREQUIRURGICAS, SET_CONVENIO_TRASLADO
} from '../actionTypes'
...
import {
    ...
    URL_GET_CONVENIO_TRASLADO,
    URL_POST_CONVENIO_TRASLADO_AJUSTAR_PORCENTAJE,
    URL_GET_CONVENIO_TRASLADO_HISTORICO,
    URL_GET_CONVENIO_TRASLADO_HISTORICO_DETALLE,
    URL_ALTA_CONVENIO_TRASLADO,
    URL_POST_EDITAR_CONVENIO_TRASLADO,
} from '../../Urls/contrataciones'

import { setLoading } from './generalConfig'
...
//GET CONVENIO - TRASLADO:
const setConvenioTraslado = (data) => {
    return {
        type: SET_CONVENIO_TRASLADO,
        payload: data
    }
}
export const getConvenioTraslado = (request, callback) => {
    return dispatch => {
        dispatch(setLoading(true))
        fetch(URL_GET_CONVENIO_TRASLADO, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(request)
        })
        .then((response) => {
            ...
            .then(data => {
                if (data.status == 200 || data.status == 204) {
                    dispatch(setConvenioTraslado(data.body))
                    dispatch(setLoading(false))
                    callback(true)
                } else {
                    dispatch(setConvenioTraslado(null))
                    dispatch(setLoading(false))
                    callback(false)
                }
            })
...
//AJUSTAR POR % - TRASLADO
export const ajustarPorcentajeConvenioTraslado = (request, callback) => {
    return dispatch => {
        dispatch(setLoading(true))
        fetch(URL_POST_CONVENIO_TRASLADO_AJUSTAR_PORCENTAJE, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(request)
        })
...
//GET CONVENIO DETALLE HISTORICO - TRASLADO
export const getConvenioTrasladoHistoricoDetalle = (req, callback) => {
    return dispatch => {
        fetch(URL_GET_CONVENIO_TRASLADO_HISTORICO_DETALLE, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(req)
        })
...
//GET CONVENIOS HISTORICOS - TRASLADO
export const getConvenioTrasladoHistorico = (req) => {
    return dispatch => {
        dispatch(setErrorConvenioHistorico(false))
        dispatch(setLoadingConvenioHistorico(true))
        fetch(URL_GET_CONVENIO_TRASLADO_HISTORICO, {
            method: "POST",
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(req)
        }).then(response => {
...
//ALTA CONVENIO - TRASLADO
export const altaConvenioTraslado = (req, cbSuccess) => {
    return () => {
        fetch(URL_ALTA_CONVENIO_TRASLADO,{
            method: "POST",
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(req)
        })
        .then(res => {
            if(res.status === 201) cbSuccess(true)
            else cbSuccess(false)
        })
        .catch(() => {
            cbSuccess(false)
        })
...
//EDITAR CONVENIO ACTUAL - TRASLADO
export const editarConvenioTraslado = (req, cbSuccess) => {
    return () => {
        fetch(URL_POST_EDITAR_CONVENIO_TRASLADO,{
            method: "POST",
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(req)
        })
        .then(res=>{
            res.json()
            .then(data=>{
                if(data.status === 200) cbSuccess(true)
                else cbSuccess(false)
            })
        })
        .catch(()=>{
